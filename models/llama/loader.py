...
handles downloading and running the actual Llama weights, this module
...
Requires Ollama installed and running locally with a Llama model pulled,
...
def load(host: str = DEFAULT_HOST, model: str = "llama3") -> OllamaClient:
    """Return an OllamaClient defaulted to the llama3 tag. Does not raise
    if the server isn't running yet - call .is_available() to check, or
    .list_models() to confirm llama3 (or your chosen model) is actually
