...
handles downloading and running the actual Gemma weights, this module
...
Requires Ollama installed and running locally with a Gemma model pulled,
...
def load(host: str = DEFAULT_HOST, model: str = "gemma2") -> OllamaClient:
    """Return an OllamaClient defaulted to the gemma2 tag. Does not raise
    if the server isn't running yet - call .is_available() to check, or
    .list_models() to confirm gemma2 (or your chosen model) is actually
