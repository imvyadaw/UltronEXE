"""
NVIDIA NIM Client with Tool Calling
===================================
2nd link in the main tool-calling fallback chain: Groq -> NVIDIA -> Gemini
-> Ollama (see ai/ai_router.py). Only used when GROQ has failed/rate-limited
for this turn.

build.nvidia.com exposes an OpenAI-compatible /v1/chat/completions endpoint,
so this client is a near-mirror of ai/cloud_models/groq_client.py, just
talking to the `openai` SDK pointed at NVIDIA_BASE_URL instead of the `groq`
SDK. Same TOOLS schema, same tool_selector/tool_runtime, same shared
conversation_history contract as every other backend ai_router.py drives.

Note: the NVIDIA example snippet many people copy uses stream=True to show
Nemotron's reasoning trace live. This client intentionally uses stream=False -
reconstructing streamed tool_call deltas correctly is a lot of extra
complexity for zero benefit inside a router that already only returns the
final text, and non-streaming keeps this file symmetric with groq_client.py.
"""
...
class UltronNvidiaClient:
    """Client for NVIDIA NIM (build.nvidia.com) - same public shape as
    UltronGroqClient so ai_router.py can drive either interchangeably."""

    def __init__(self):
        if not NVIDIA_API_KEY:
            raise ValueError("NVIDIA_API_KEY not found! Set it in .env to enable the NVIDIA fallback.")
        ...
        self.client = OpenAI(
            base_url=NVIDIA_BASE_URL, api_key=NVIDIA_API_KEY, timeout=CLOUD_CLIENT_TIMEOUT_SECONDS, max_retries=0
        )
    ...
    def chat_with_tools(self, user_message: str, tool_callback=None) -> str:
        ...
        try:
            ...
            print(f"Model nvidia/{self.model} failed ({reason}). Switching to next backend...")
            return f"Error: NVIDIA model failed ({reason}): {e}"
...
nvidia_client: Optional["UltronNvidiaClient"] = None


def get_nvidia_client() -> UltronNvidiaClient:
    global nvidia_client
    if nvidia_client is None:
        nvidia_client = UltronNvidiaClient()
