...
api.deepseek.com exposes an OpenAI-compatible /v1/chat/completions endpoint,
so this client is a near-mirror of ai/cloud_models/nvidia_client.py, just
talking to the `openai` SDK pointed at DEEPSEEK_BASE_URL. Same TOOLS schema,
same tool_selector/tool_runtime, same shared conversation_history contract
as every other backend ai_router.py drives.
"""
...
class UltronDeepSeekClient:
    """Client for DeepSeek (api.deepseek.com) - same public shape as
    UltronGroqClient/UltronNvidiaClient so ai_router.py can drive any of
    them interchangeably."""

    def __init__(self):
        if not DEEPSEEK_API_KEY:
            raise ValueError("DEEPSEEK_API_KEY not found! Set it in .env to enable the DeepSeek fallback.")
        ...
        self.client = OpenAI(
            base_url=DEEPSEEK_BASE_URL, api_key=DEEPSEEK_API_KEY, timeout=CLOUD_CLIENT_TIMEOUT_SECONDS, max_retries=0
        )
    ...
    def chat_with_tools(self, user_message: str, tool_callback=None) -> str:
        ...
        try:
            ...
            print(f"Model deepseek/{self.model} failed ({reason}). Switching to next backend...")
            return f"Error: DeepSeek model failed ({reason}): {e}"
        ...
        while getattr(message, "tool_calls", None) and rounds < MAX_TOOL_ROUNDS:
            ...
            # (I/O-bound tools, order preserved, per-job error isolation).
            parsed_calls = []
...
deepseek_client: Optional["UltronDeepSeekClient"] = None


def get_deepseek_client() -> UltronDeepSeekClient:
    global deepseek_client
    if deepseek_client is None:
        deepseek_client = UltronDeepSeekClient()
