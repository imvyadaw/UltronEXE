...
Chrome downloads
==================
Chrome-specific download history straight from its own SQLite History
file, plus the shared Downloads-folder operations (list/open/clean up)
that apply regardless of which browser did the downloading. For a
single call that covers Chrome, Edge, and Firefox together, see
browser/automation/download_manager.py.
"""
...
class ChromeDownloads:
    """List/open/clean up downloads, plus Chrome's own download history."""
    ...
    def clear_downloads(self, older_than_days: Optional[int] = None, confirm: bool = False) -> Dict:
        """Delete files from Downloads. If older_than_days is set, only
        deletes files older than that; otherwise deletes everything.
        Safety-gated: needs confirm=true (permanent delete, not Recycle Bin)."""
        if not confirm:
    ...
    def download_history(self, limit: int = 20) -> Dict:
        ...
        from Chrome's History SQLite file. Copies the file first since
        it's locked while Chrome is running."""
        try:
            ...
            return {"browser": "chrome", "count": len(downloads), "downloads": downloads}
        except Exception as e:
            return {"error": str(e)}

    def search_downloads(self, query: str, limit: int = 20) -> Dict:
        """Search Chrome's download history for a filename/URL substring."""
        ...
        return {"query": query, "browser": "chrome", "count": len(matches), "matches": matches}
